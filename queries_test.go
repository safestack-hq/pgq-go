...
func TestEnqueueJob(t *testing.T) {
	tt := []struct {
	...
	}{
		{
			...
			makeAssertions: func(t *testing.T, tx *sqlx.Tx, id int, err error) {
				...
				assert.Equal(t, "some data", job.Data)
	...
	for _, tc := range tt {
		t.Run(tc.desc, func(t *testing.T) {
			tx, _ := db.Beginx()
			defer tx.Rollback()
			id, err := enqueueJob(tx, "someQueue", "some data", tc.options...)
	...
	for _, tc := range tt {
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.jobExpected {
				assert.Nil(t, err)
				assert.NotNil(t, job)
				assert.Equal(t, tc.expectedData, fmt.Sprintf("%v", job.Data))
			}
		})
	}
}
...
func TestUpdateJob(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, "update bar", fmt.Sprintf("%v", fetchedJob.Data))
